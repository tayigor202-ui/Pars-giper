...
def print_final_instructions():
    ...
    print(f"  Запуск парсера Ozon:       {Colors.OKCYAN}python parsers/ozon_parser_production_final.py{Colors.ENDC}")
    print(f"  Запуск парсера WB:         {Colors.OKCYAN}python parsers/wb_parser_production.py{Colors.ENDC}")
    print(f"  Генерация отчёта Ozon:     {Colors.OKCYAN}python scripts/generate_report.py{Colors.ENDC}")
    print(f"  Генерация отчёта WB:       {Colors.OKCYAN}python scripts/wb_reporting.py{Colors.ENDC}")
    print(f"  Тест Telegram:             {Colors.OKCYAN}python scripts/send_report_to_telegram.py{Colors.ENDC}\n")
...
def main():
    ...
    try:
        # Run the direct setup script (in same folder as setup.py)
        ...
        if os.path.exists(setup_script):
            ...
            print_error(f"Файл {setup_script} не найден. База данных не инициализирована.")
    except Exception as e:
